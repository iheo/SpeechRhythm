...
%                   .p : feature vector parameters
        ...
        % Gender Parameter
        CIdx{1} = [1, 2];
        Gmm(1).idx = find(IsTest.IsMale);   % Pr( x | Male )
        Gmm(2).idx = find(IsTest.IsFemale); % Pr( x | Female)

        % Nativeness Parameter
        CIdx{2} = [3, 4];
        Gmm(3).idx = find(IsTest.IsNative);   % Pr( x | Native )
        Gmm(4).idx = find(IsTest.IsNonNative); % Pr( x | NonNative )
